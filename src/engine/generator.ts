...
const WORLD_LOCALES = ['pt-BR', 'en-US', 'en-GB', 'fr-FR', 'en-IE'];
...
class FakeDataGenerator {
  ...
  generate(fields: string[], count = 10, customFields: CustomField[] = []) {
    if (this.locale === 'world') {
      const savedLocale = this.locale;
      const result = Array.from({ length: count }, () => {
        this.locale = randomPick(WORLD_LOCALES);
        return this.generateOne(fields, customFields);
      });
  ...
  generateOne(fields: string[], customFields: CustomField[] = []) {
    ...
    if (hasCountry) {
      ...
      const defaultKey = this.locale.startsWith('en') ? (this.locale === 'en-GB' ? 'GB' : this.locale === 'en-IE' ? 'IE' : 'US') : this.locale === 'pt-BR' ? 'BR' : this.locale === 'fr-FR' ? 'FR' : randomPick(keys);
